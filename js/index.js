...
var vm = new Vue({
	...
	methods : {
		init:function(){
			var _this = this;
			this.$http.get('./json/mock.json').then(function(res){
		...
		endMove(e){
			...
			this.$refs.desk.forEach(function(item,index){
				...
				if(_this.isInArea(_this.moveingTarget.getBoundingClientRect(),item.getBoundingClientRect())){
					...
					console.log(index)
					var moveOldStudent = _this.classInfo.desk_list[_this.moveingItem].student;
					var moveNewStudent = _this.classInfo.desk_list[index].student;
					_this.classInfo.desk_list[index].student = moveOldStudent;
					_this.classInfo.desk_list[_this.moveingItem].student = moveNewStudent;
